...
* **Figma Text Styles support — staged, see the decision above for the shape.**
  ...
  * ~~Stage 0: verify `fontWeight` vs `fontStyle` in a real Figma file~~ **Done July 2026** — `fontStyle` is correct, see the decision above. No longer blocking.
  * Stage 2: read `getLocalTextStylesAsync()` → tokens (new `plugin/src/plugin/figma-text-styles.ts`).
  * Stage 3: write tokens → `createTextStyle`/`setBoundVariable`, after the underlying leaf Variables exist (same ordering constraint as Primitives-before-Semantic). Unblocked.

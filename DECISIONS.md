...
* **Unresolved before implementation continues**: Figma's `fontWeight` vs `fontStyle` bindable fields are functionally undocumented beyond the type union — no Figma doc found gives an example of what binding each one actually does. Must be verified against a real Figma file before Stage 3 (write path) is trusted. See Planned below.
...
* **Figma Text Styles support — staged, see the decision above for the shape.**
  ...
  * **Stage 0 (blocking Stage 3): manually verify in a real Figma file which of `fontWeight`/`fontStyle` actually does what** when bound via `TextStyle.setBoundVariable` — undocumented beyond the type signature. No Figma access this session; needs a human to run a small spike and report back.
  * Stage 2: read `getLocalTextStylesAsync()` → tokens (new `plugin/src/plugin/figma-text-styles.ts`).
  * Stage 3: write tokens → `createTextStyle`/`setBoundVariable`, after the underlying leaf Variables exist (same ordering constraint as Primitives-before-Semantic). Blocked on Stage 0.
  * Stage 4: wire into the `UIMessage`/`PluginMessage` protocol and `Sync.tsx`, following the `GET_COLLECTIONS`/`APPLY_TOKENS` pattern.
  * Deliberately deferred: a new diff UI (the underlying leaf tokens already diff as today); Effect Styles/shadow (identical pattern, needs its own `VariableBindableEffectField` research, zero current usage in this repo's tokens).
  * Verification: a lightweight `figma` global mock for vitest is needed for real apply-path tests (doesn't exist yet — also the reason `figma-variables.ts`'s apply logic itself has never been directly tested). Manual Figma verification remains the actual correctness authority regardless.

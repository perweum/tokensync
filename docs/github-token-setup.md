# Setting up a GitHub token for Token Spark

Token Spark needs a GitHub Personal Access Token (PAT) to read and write your
...
Token Spark actually uses. If it's ever exposed, the blast radius is one repo,
not your entire account.

## What Token Spark actually needs
...
Nothing else. Token Spark never touches Issues, Actions, Packages, Webhooks,
...
2. Give it a name that identifies what it's for (e.g. `token-spark-<your-repo-name>`).
...
7. Paste it into Token Spark's project setup (**GitHub Personal Access
...
Token Spark with the new token, or remove the project if you no longer need
the connection.

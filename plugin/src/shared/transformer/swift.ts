...
function swiftFontWeight(value: string): string {
  ...
  if (resolved === null) {
    console.warn(`[TokenSpark] Unrecognized font weight "${value}" — using Font.Weight.regular`);
  }
...
export function generateSwift(collections: ResolvedCollection[], metadata: Metadata): string {
  ...
  for (const col of themes) {
    blocks.push(swiftStruct("DesignTokensTheme" + toModeTypeName(col.modeName), col.tokens));
  }
  for (const col of semantic) {
    const structName = "DesignTokens" + toModeTypeName(col.modeName);

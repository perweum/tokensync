      ...
      {!hasChanges ? (
        ...
        <>
          ...
          {(() => {
            ...
            if (!diff || diff.counts.total === 0) return (
    ...
    <span style={{ display: 'flex', alignItems: 'center', gap: '3px', opacity: faded ? 0.4 : 1 }}>
      {isColor && hex && (
        <span style={{
          ...
          background: value, border: light ? '1px solid #ddd' : '1px solid transparent',
        }} />
      )}
...
const s: Record<string, React.CSSProperties> = {
  ...
  overview:     { display: 'flex', gap: '16px', padding: '12px 16px', borderBottom: '1px solid #eee', alignItems: 'center', flexWrap: 'wrap' },
  ...
  prInput:      { border: '1px solid #ddd', borderRadius: '6px', padding: '8px 10px', fontSize: '12px', outline: 'none' },
  prBtn:        { background: '#1a52d8', color: '#fff', border: 'none', borderRadius: '8px', padding: '11px', fontSize: '13px', fontWeight: 500, cursor: 'pointer' },

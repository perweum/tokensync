...
export async function applyTokensToCollection(
  ...
  let mode =
    collection.modes.find((m) => m.modeId === modeId) ??
  ...
  const collectionVars = new Map(
    allLocalVars.filter((v) => v.variableCollectionId === collection.id).map((v) => [v.name, v]),
  );
  ...
  if (cleanApply) {
    ...
    for (const existingMode of [...collection.modes]) {
      ...
      try {
        ...
        /* ignore — can't remove last mode or already gone */
      }
    }
  ...
  for (const path of sortedPaths) {
    ...
    try {
      ...
      const literalValue = hasRef(rawValue) ? resolveInlineRefs(rawValue, allVarsByName) : rawValue;
      ...
      let valueToUse =
        isPureRef(rawValue) && !hasRef(literalValue)
          ? literalValue
          : hasRef(literalValue)
            ? (resolvedFlat[path]?.$value ?? literalValue)
            : literalValue;
 ...
 */
function resolveInlineRefs(value: string, allVarsByName: Map<string, Variable>): string {
